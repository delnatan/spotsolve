...
/// The local fit around one candidate: `(free, frozen, bbox)`.
///
/// **`free` is ordered by distance to the candidate, and that order is
/// load-bearing** -- it decides the theta layout of the fit, hence which
/// emitter ends up at which offset when the result is written back. `frozen` is
/// in ascending index order, matching `np.setdiff1d`.
pub fn window(
    pos: &[f64],
    n: usize,
    cand_y: f64,
    ...
    grid: &EmitterGrid,
    scratch: &mut Vec<u32>,
) -> (Vec<u32>, Vec<u32>, BBox) {
    let link_r = LINK_FACTOR * sigma;
    let mut free: Vec<u32> = Vec::new();
    if n > 0 {
        grid.query_disc(pos, cand_y, cand_x, link_r, scratch);
        free = scratch.clone();
        // Nearest first, index as tiebreak; then capped so the joint Fisher
        // matrix stays small.
        free.sort_by(|&a, &b| {
    ...
    let mut ys: Vec<f64> = free.iter().map(|&i| pos[2 * i as usize]).collect();
    let mut xs: Vec<f64> = free.iter().map(|&i| pos[2 * i as usize + 1]).collect();
    ys.push(cand_y);
    xs.push(cand_x);
    let bbox = bbox_for(&ys, &xs, BBOX_PAD * sigma, h, w);

    let frozen = if n > 0 {
        frozen_around(pos, grid, &bbox, HALO_FACTOR * sigma, &free, scratch)
    } else {
        Vec::new()
    };
    (free, frozen, bbox)
...
#[inline]
fn dist2(pos: &[f64], i: usize, y: f64, x: f64) -> f64 {
    let (dy, dx) = (pos[2 * i] - y, pos[2 * i + 1] - x);
    dy * dy + dx * dx
}
